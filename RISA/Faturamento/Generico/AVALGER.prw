...
User Function DADOSAPRO()
	...
	cQuery += " INNER JOIN " + RetSQLName("DA1") + " DA1 ON DA1.D_E_L_E_T_ <> '*' AND DA1.DA1_FILIAL = '"+xFilial("DA1")+ "' AND DA1.DA1_CODPRO = SC6.C6_PRODUTO AND DA1_CODTAB = SC5.C5_TABELA " + ENTER
	...
	cQuery += " INNER JOIN " + RetSQLName("SZC") + " SZC ON SZC.D_E_L_E_T_ <> '*' AND SZC.ZC_FILIAL  = '"+xFilial("SZC")+ "' AND SC5.C5_VEND1 = SZC.ZC_VEND AND SB1.B1_GRUPO >= SZC.ZC_GRPROD AND SB1.B1_GRUPO <= SZC.ZC_GRPRODA " + ENTER
	cQuery += " INNER JOIN " + RetSQLName("SBM") + " SBM ON SBM.D_E_L_E_T_ <> '*' AND SBM.BM_FILIAL  = '"+xFilial("SBM")+ "' AND SBM.BM_GRUPO = SZC.ZC_GRPROD " + ENTER
	cQuery += " WHERE " + ENTER
