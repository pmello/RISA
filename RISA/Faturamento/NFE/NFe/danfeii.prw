...
User Function PrtNfeSef(cIdEnt, cVal1		, cVal2		, oDanfe,;
						oSetup, cFilePrint	, lIsLoja	, nTipo, lAutomato )
...
If nTipo <> 1
	...
	Else
		...
		If lVerPerg
            ...
            //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
				...
				If IIF(ISINCALLSTACK("U_GCFIS002"), (Pergunte("NFSIGW",.F.), .T.), lPergunte := Pergunte("NFSIGW",.T.))
	...
	If lPergunte
		if lAutomato
			...
			RPTStatus( {|lEnd| U_DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja,,lAutomato )}, "Imprimindo DANFE..." )
		endif
	EndIf
...
ElseIf nTipo == 1
	...
	If lPergunte
		...
		if lAutomato
			U_DANFEProc(@oDanfe, , cIDEnt, Nil, Nil, @lExistNFe, lIsLoja, nTipo,lAutomato )
		else
			RPTStatus( {|lEnd| U_DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja, nTipo, lAutomato)}, "Imprimindo DANFE..." )
		endif
...
User Function DANFEProc(	oDanfe	, lEnd		, cIdEnt	, cVal1,;
							cVal2	, lExistNfe	, lIsLoja	, nTipo, lAutomato)
	...
	While  !lBreak  .And. nLenarray >= 0
		...
		While !Eof() .And. !lBreak .And. ;
			...
			For nx :=1 To nLenNotas
				...
				MsSeek(cChaveSFT)
				If ( !Empty(aXML[nX][2]) .AND. (AllTrim((cAliasSFT)->FT_ESPECIE)== 'SPED') .Or. (lImpDir .And. !Empty(aXML[nX][2])) ) .And. Empty((cAliasSFT)->FT_DTCANC)
					If !Empty(aXml[nX])
						cAutoriza   := aXML[nX][1]
						cCodAutDPEC := aXML[nX][5]
						cCodRetNFE	:= aXML[nX][9]
...
ElseIf nTipo == 1
	U_IMPDNFLJ(oNFE, cCodAutSef, dDtReceb, cDtHrRecCab)
EndIf
		...
		*/
		...
		For nA := 1 To LEN( aInfCpl )
			...
			While !Empty( cAux )
				...
				cAux := SubStr( cAux,( IIF( Left( cAux, 7 ) <> "!NOBRK!", nMaxDes, TamSX3( 'DY3_DESCRI' )[ 1 ] ) + 1 ) )
		    		lPontilhado := .T.
			EndDo
		Next nA		
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
		//³FIM TRECHO ESPECIFICO RISA³
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	// else
		aTamCol := {50, 150, 40, 16, 24, 14, 35, 53, 38, 36, 30, 20, 31, 24} //Valores fixados, pois localizado erro de impressão quando visualização de S.O. é diferente de 100%
	// endif
