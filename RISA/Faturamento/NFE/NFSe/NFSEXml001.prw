	...
	If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			aadd(aProd,	{Len(aProd)+1,;
							...
							SF3->F3_ISSSUB,;
							SF3->F3_ISSMAT,;
							IIF(lCrgTrib .And. cTpCliente == "F",IIF((cAliasSD2)->(FieldPos("D2_TOTIMP"))<>0,(cAliasSD2)->D2_TOTIMP,0),0),; //Carga Tributária - Lei transparência
		...
		If SF1->(FieldPos("F1_INSS"))<>0 .and. SF1->F1_INSS>0
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				aadd(aProd,	{Len(aProd)+1,;
								...
								SF3->F3_ISSSUB,;
								SF3->F3_ISSMAT,;
								IIF(lCrgTrib .And. cTpCliente == "F",IIF((cAliasSD1)->(FieldPos("D1_TOTIMP"))<>0,(cAliasSD1)->D1_TOTIMP,0),0),; //Carga Tributária - Lei transparência
								IIF(Empty(SC6->C6_CODISS),SB1->B1_CODISS,SC6->C6_CODISS),;
								0,;
								}) 
...
Static Function NFSETransp(cCodMun)
Local cString := ""
...
Função para retorno do Tamanho da Inscrição Municipal
...
@return		nTam			Tamanho da Inscrição a ser configurada.
@obs		
/*/
//-----------------------------------------------------------------------
...
Do Case
	Case cCodMun $ "2211001-1501402"	// PI-Teresina e PA-Belem 
		nTam:= 7
	Case cCodMun $ "5002704-2111300"	// MS-Campo Grande e MA-São Luis
		nTam:= 11
