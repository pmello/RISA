		...
		While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			If !lAglutina .Or. Len(aProd) == 0
				...
				aadd(aProd,	{Len(aProd)+1,;
							...
							IIF((cAliasSD2)->D2_BASEISS <> nValTotPrd, nValTotPrd - (cAliasSD2)->D2_BASEISS, (cAliasSD2)->D2_BASEISS),; //Posicao para verifcar se existe reducao de ISS, será criado um campo na SFT para substituir esse calculo
...
ElseIf cCodMun $ "3550308" //São Paulo 
	Do Case
		Case aTotal[3] $ "1"    //Tributação no municipio
			cTributa := "1" 
	    Case aTotal[3] $ "2"    //Tributação fora do municipio
...
Static Function NFSETransp2(cCodMun)
Local cString := ""
